...
scene_description = {
    ...
    'light': {
        ...
        "to_world": mi.ScalarTransform4f().look_at(
                ...
                target = [0, 0,  -1],
                up=[0, 0, 1]
            ).scale(scale[1]) @ T().rotate([0, 0, 1], -90),
        'emitter': {
            'type': 'area',
            'radiance': {
                #'type': 'rgb',
...
key1 = 'plane.to_world'
key2 = 'light.to_world'
initial_params1 = mi.Transform4f(params[key1])
initial_params2 = mi.Transform4f(params[key2])

def update(theta):
    #params[key1] = dr.ravel(initial_params1 + mi.Vector3f(0, 0, theta))
    #params[key2] = dr.ravel(initial_params2 + mi.Vector3f(0, 0, theta))

    params[key1] =  mi.Transform4f().translate(mi.Vector3f(0, 0, theta)) @ initial_params1
    params[key2] =  mi.Transform4f().translate(mi.Vector3f(0, 0, theta)) @ initial_params2
...
if generate_prb_projective:
    ...
    prb_projective_fwd = prb_projective_integrator.render_forward(scene, params=params)
    mi.Bitmap(prb_projective_fwd).write('prb_projective_fwd.exr')
