...
**shear**, a **rotation** (quaternion) and a **translation**. Evaluating the
animation at some time interpolates rotations using `spherical linear
interpolation (Slerp)
<https://en.wikipedia.org/wiki/Spherical_linear_interpolation>`_ and everything
else linearly.
...
* Only affine transformations can be represented. Shear is supported, but the
  ``perspective`` and ``thinlens`` sensors reject it in their ``to_world``
  (along with scale factors), animated or not.
...
(:monosp:`resources/data/docs/scenes/animation_sensor.xml`).
...
(:monosp:`resources/data/docs/scenes/animation_emitter.xml`). The geometry stays sharp while
...
(:monosp:`resources/data/docs/scenes/animation_shape.xml`).

.. note::

    Animation of shapes requires **evenly spaced** keyframes: the Embree, OptiX
    and Metal APIs all describe motion as a keyframe count plus a time range,
    and derive the intermediate times by subdividing it. An instance whose
    keyframes are not uniformly spaced is rejected with an error. This
    restriction does not apply to sensors and emitters.
...
.. list-table::
    ...
    * - Embree (``llvm_*``, ``scalar_*``)
      - Slerp
      - Fully supported; matches Mitsuba's own evaluation. An instance covering
        only part of the scene-wide keyframe range is padded out to it, which
        caps the usable keyframe count at 129 time steps *after* padding.
    * - OptiX (``cuda_*``)
      - Nlerp
    ...
    * - Metal (``metal_*``)
      - Nlerp
      - Fully supported; uses component transforms (``MTLComponentTransform``)
        to interpolate decomposed scale, quaternion rotation (normalized lerp),
        and translation, matching OptiX.
    * - Native kd-tree
      - --
      - Instancing is unsupported altogether, so the scene is rejected with an
        error before animation matters.

The scene below makes the differences between Slerp and Nlerp visible. A teapot
rotates 150 degrees about the vertical axis between two keyframes, and the shutter is
*closed* (``shutter_open == shutter_close``) so that each image is the instantaneous
pose rather than a blur (:monosp:`resources/data/docs/scenes/animation_rotation.xml`).
...
how fast they traverse it, so the blurs cover the same region and differ
only in density -- visible in the smeared spout, but easy to miss. Both GPU backends
(CUDA and Metal) match closely here.

Nlerp and slerp follow the same rotation path, but nlerp traverses it at a
non-constant angular rate, so a GPU render (CUDA or Metal) can differ slightly from an
LLVM one for instances that rotate a lot between two keyframes. The deviation depends
...
Both OptiX and Metal use component transforms with normalized linear quaternion
interpolation (nlerp), ensuring consistent behavior across GPU backends.
...
five tensors, one per interpolated component:
...
The five tensors are views into a single packed buffer, so they must always
agree on the number of keyframes. **Changing the number of keyframes therefore
means writing all five together**, with a matching row count; writing only some
...
.. code-block:: python
    ...
    params['shear']       = mi.TensorXf([0.0] * 6, shape=(2, 3))
...
the plain 4x4 matrix directly under its parent name (e.g. ``to_world``). That
matrix is the representation actually evaluated in that case, and it takes
precedence if it is written together with the views:
