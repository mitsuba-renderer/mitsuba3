/*
    ...
    Combined buffer layout: Mitsuba concatenates the per-primitive data of
    every fixed-size shape of a given type (sphere / disk / cylinder /
    ellipsoids) into ONE buffer per type, bound at the type's IFT slot
    ([[buffer(N)]] where N = INTERSECTION_FN_<type>). SDFGrid data is
    shape-level (header + voxel indices + sample grid) and variable-length;
    the slices of all SDFGrids are concatenated (16-byte aligned) into one
    combined buffer at INTERSECTION_FN_SDFGRID's slot.

    The argument table of a Metal intersection function table is shared by
    ALL entries -- there are no per-entry bindings ("last write wins" per
    slot) -- so a geometry's slice cannot be communicated through its IFT
    entry. Instead, a scene-global lookup table bound at
    [[buffer(INTERSECTION_FN_COUNT)]] maps each (instance, geometry) pair
    to its slice: the first n_instances words hold every TLAS instance's
    base index into a per-BLAS record region that follows, with one word
    per geometry in BLAS order (see metal_accel.mm). Fixed-size types
    interpret the looked-up word as an *element* index into their typed
    combined buffer; SDFGrid interprets it as a *byte* offset. This scheme
    works uniformly for top-level shapes and shape-group children.
    (Per-primitive data would be a natural alternative carrier for the
    offsets, but Apple's MSL rejects the [[primitive_data]] spellings we
    tried for bounding-box intersection functions.)
...
BoundingBoxIntersection intersection_sphere(
    ...
    device const uint *lookup           [[buffer(5)]])
{
