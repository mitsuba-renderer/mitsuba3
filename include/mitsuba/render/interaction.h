...
struct Interaction {
    ...
    Interaction(Float t, Float time, const Wavelength &wavelengths,
        ...
        : t(t), time(time), wavelengths(wavelengths), p(p), shadow_offset(0.f), n(n) { }
    ...
    virtual void zero_(size_t size = 1) {
        ...
        shadow_offset = dr::zeros<Vector3f>(size);
    ...
    DRJIT_STRUCT(Interaction, t, time, wavelengths, p, n, shadow_offset);
...
private:
    ...
    Point3f offset_p(const Vector3f &d) const {
        ...
        return dr::fmadd(mag, dr::detach(n), p + shadow_offset);
    }
};
...
struct SurfaceInteraction : Interaction<Float_, Spectrum_> {
    ...
    MI_IMPORT_BASE(Interaction, t, time, wavelengths, p, shadow_offset, n, is_valid)
    ...
    DRJIT_STRUCT(SurfaceInteraction, t, time, wavelengths, p, shadow_offset, n,
...
struct MediumInteraction : Interaction<Float_, Spectrum_> {
    ...
    DRJIT_STRUCT(MediumInteraction, t, time, wavelengths, p, shadow_offset, n, medium,
