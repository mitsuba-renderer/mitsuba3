 ...
 *        (See \ref ray_intersect_preliminary(), \ref ray_intersect(),
 *        and \ref ray_test()).</li>
 *
...
public:
     ...
     *
     * \param ray
     *    A 3D ray including maximum extent (\ref Ray::maxt) and time
     *    (\ref Ray::time) information, which matters when the shapes are in motion
     *
     * \return
     ...
     *
     * <ol>
     *   <li> Finding a `PreliminaryIntersection3f` using the ray tracing
     ...
     *   </li>
     *
     *   <li> Expanding the `PreliminaryIntersection3f` into a full
     ...
     *
     * \param ray_flags
     *    An integer combining flag bits from `RayFlags` (merged using
     *    binary or).
     *
     * \param coherent
     ...
     * \ref ray_intersect() method will re-evaluate certain parts of the computation
     ...
     * \ref Sensor::sample_ray(), which means that adjacent rays will traverse
    ...
    /**
     * \brief Evaluate the discrete probability of the
     * \ref sample_emitter() technique for the given a emitter index.
     */
     ...
     *    A uniformly distributed sample on the domain <tt>[0,1]^2</tt>.
     *
     * \return A tuple <tt>(ray, weight, emitter)</tt>, where
     *    <ul>
     ...
     *    <ul>
     *      <li>\c ds is a fully populated `DirectionSample3f` data
     ...
     * \ref sample_emitter_direction() function. The returned probability will always
    ...
    /**
     * \brief Re-evaluate the incident direct radiance of the
     * \ref sample_emitter_direction() method.
     *
     ...
     *      `SilhouetteSample3f` field \c discontinuity_type will be
