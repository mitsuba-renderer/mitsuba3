...
// Nanobind type caster for mitsuba::field<...> types.
// The type caster "unwraps" the device value when going from C++ to Python, and
// conversely wraps it into field when going from Python to C++.
...
struct type_caster<mitsuba::field<DeviceType, HostType>> {
    ...
    static handle from_cpp(T &&value, rv_policy policy,
                           cleanup_list *cleanup) noexcept {
        return Caster::from_cpp(forward_like_<T>(value.value()), policy, cleanup);
    }
