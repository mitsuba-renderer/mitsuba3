...
static const char *__doc_mitsuba_Bitmap_channel_names = R"doc(Return the names of the channels used by this bitmap)doc";
...
static const char *__doc_mitsuba_Bitmap_pixel_format_from_channels =
R"doc(Return the pixel format described by a set of channel names

The order of the names does not matter. Names that don't spell out one of
the color formats (e.g. ``R``, ``G``, ``B``, ``A``) yield
``PixelFormat::MultiChannel``.)doc";
...
R"doc(Abstract film base class - used to store samples generated by `Integrator`
implementations.

Each sample contributes to nearby pixels according to the film's
reconstruction filter. With randomly placed samples, the total filter
weight received by a pixel fluctuates. Dividing by a fixed normalization
constant would therefore produce noise even in trivial scenes, such as a
flat surface lit by a point light. The film instead accumulates the filter
weights along with the samples and divides by their sum when developing the
image. This removes the noise at the cost of a small bias that vanishes as
the sample count increases, i.e., the estimate is consistent.
...
samples into a single block covering the film.)doc";
...
static const char *__doc_mitsuba_Film_divide_by_weight = R"doc(Divide the channels of a storage tensor by its trailing weight channel)doc";
...
static const char *__doc_mitsuba_Film_m_pixel_weight = R"doc(Total filter weight of every pixel, or zero if 'm_storage' has a weight channel)doc";
...
static const char *__doc_mitsuba_Film_pixel_weight =
R"doc(Return the total filter weight of every pixel passed to `prepare()`,
or zero when the film stores a weight channel)doc";
...
R"doc(Convert a spectral sample to the film's base channels.
...
Returns:
    A list with one value per base channel (see `base_channels()`).)doc";
...
static const char *__doc_mitsuba_Film_sensor_response_function =
R"doc(Return the Sensor Response Function (SRF) of spectral films, which
sensors use to sample wavelengths. Returns ``nullptr`` for other films.)doc";
...
static const char *__doc_mitsuba_Film_storage =
R"doc(Return the accumulated samples, i.e. the channels of the developed
image followed by the weight channel ``W``, prior to normalization)doc";
