...
private:
    ...
    void compute_cdf_scalar(const ScalarFloat *nodes, size_t size_nodes,
        ...
        std::vector<ScalarFloat> normalization(size_cond, 0.f);
        ...
        for (size_t i = 0; i < size_cond; i++) {
            ...
            normalization[i] = 1 / integral_val;
        }
...
public:
    ...
    ConditionalRegular1D(const FloatStorage &pdf, const ScalarVector2f &range,
        ...
        if (m_size_nodes < 2)
            Log(Error,
                "The number of final dimension should have at least size 2 "
                "instead of %u",
                m_size_nodes);
    ...
    void update() {
        ...
        for (size_t i = 0; i < m_range_cond.size(); i++) {
            ...
            m_inv_interval_cond.push_back(dr::rcp(tmp));
        }
