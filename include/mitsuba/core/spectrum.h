 ...
 */
    template<typename T>
    auto depolarizer(const T &spectrum = T(1)) {
        if constexpr (is_polarized_v<T>) {
            T result = ek::zero<T>();
...
/// Convert XYZ tristimulus values to ITU-R Rec. BT.709 linear RGB
    ...
    template<typename Float>
    Float luminance(const Color<Float, 3> &c) {
 ...
 */
    template<typename Value>
    Value pdf_rgb_spectrum(const Value &wavelengths) {
