...
NAMESPACE_BEGIN(util)
    ...
    extern std::string MTS_EXPORT_LIB last_error();
#endif

/// Determine the number of available CPU cores (including virtual cores)
extern MTS_EXPORT_LIB int core_count();
 ...
 * is generated.
 */
extern MTS_EXPORT_LIB std::string time_string(float time, bool precise = false);

/// Turn a memory size into a human-readable string
extern MTS_EXPORT_LIB std::string mem_string(size_t size, bool precise = false);

/// Returns 'true' if the application is running inside a debugger
extern MTS_EXPORT_LIB bool detect_debugger();

/// Generate a trap instruction if running in a debugger; otherwise, return.
extern MTS_EXPORT_LIB void trap_debugger();

/// Return the absolute path to <tt>libmitsuba-core.dylib/so/dll<tt>
extern MTS_EXPORT_LIB fs::path library_path();

/// Determine the width of the terminal window that is used to run Mitsuba
extern MTS_EXPORT_LIB int terminal_width();

/// Return human-readable information about the Mitsuba build
extern MTS_EXPORT_LIB std::string info_build(int thread_count);

/// Return human-readable information about the version
extern MTS_EXPORT_LIB std::string info_copyright();

/// Return human-readable information about the enabled processor features
extern MTS_EXPORT_LIB std::string info_features();
