...
#define MTS_ROUTE_MODE(mode, function, ...)                                 \
    [&]() {                                                                 \
        ...
        else if (mode == "packet_rgb")                                      \
            return function<Packet<float>, Color<Packet<float>, 3>>(__VA_ARGS__); \
        else                                                                \
...
template <typename Float, typename Spectrum_> constexpr const char *get_variant() {
    ...
    else if constexpr (std::is_same_v<Float, Packet<float>> && std::is_same_v<Spectrum_, Color<Packet<float>, 3>>)
        return "packet_rgb";
    else
        return "";
}
