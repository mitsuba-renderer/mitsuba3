...
def write_core_config_cpp(f, enabled, default_mode):
    def w(s):
        f.write(s.ljust(79) + ' \\\n')
...
def write_core_config_python(f, enabled, default_mode):
    f.write('""" This file is automatically generated from "mitsuba.conf" using the script\n')
    f.write('    "resources/configure.py". Please do not attempt to change it manually,\n')
    f.write('    as any changes will be overwritten."""\n\n')

    f.write("PYTHON_EXECUTABLE='{PYTHON_EXECUTABLE}'\n".format(PYTHON_EXECUTABLE=sys.executable))
    f.write('MTS_VARIANTS={list}\n'.format(list=str([v[0] for v in enabled])))
...
if __name__ == '__main__':
    ...
    output = StringIO()
    write_core_config_cpp(output, enabled, default_mode)
    write_to_file_if_changed(fname, output.getvalue())

    fname = realpath(join(root, 'src/python/config.py'))
    output = StringIO()
    write_core_config_python(output, enabled, default_mode)
    write_to_file_if_changed(fname, output.getvalue())
