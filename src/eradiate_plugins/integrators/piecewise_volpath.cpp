...
This plugin provides a piecewise volumetric path tracer that can be used to compute approximate 
solutions of the radiative transfer equation for plane parallel geometry. Its implementation 
makes use of multiple importance sampling to combine BSDF and phase function sampling with 
direct illumination sampling strategies. On surfaces, it behaves exactly like the standard path tracer. 

This integrator only works with piecewise media. It uses assumptions on the medium's geometry to 
accelerate the sampling and transmission calculations. 
...
class PiecewiseVolumetricPathIntegrator : public MonteCarloIntegrator<Float, Spectrum> {
...
public:
    ...
    std::pair<Spectrum, Mask> sample(const Scene *scene,
        ...
        while (loop(active)) {
            ...
            if (dr::any_or<true>(active_medium)) {
                ...
                std::tie( mei, tr, pdf ) = medium->sample_interaction_real(ray, si, sampler->next_1d(active_medium), channel, active_medium);
    ...
    sample_emitter(const Interaction &ref_interaction, const Scene *scene,
        ...
        while (loop(dr::detach(active))) {
            ...
            dr::masked(total_dist, active_medium) += dr::minimum(ray.maxt, si.t);
            ...
            if (dr::any_or<true>(active_medium)) {
                ...
                if (dr::any_or<true>(is_spectral)) {
                    ...
                    UnpolarizedSpectrum free_flight_pdf = dr::zeros<UnpolarizedSpectrum>();

                    std::tie(tr, free_flight_pdf, escaped_medium) = medium->eval_transmittance_pdf_real(ray, si, channel, active_medium);
                    Float tr_pdf = index_spectrum(free_flight_pdf, channel);
                    dr::masked(transmittance, is_spectral) *= dr::select(tr_pdf > 0.f, tr, 0.f); //exact estimation
                }
...
MI_IMPLEMENT_CLASS_VARIANT(PiecewiseVolumetricPathIntegrator, MonteCarloIntegrator);
MI_EXPORT_PLUGIN(PiecewiseVolumetricPathIntegrator, "Piecewise Volumetric Path Tracer integrator");
