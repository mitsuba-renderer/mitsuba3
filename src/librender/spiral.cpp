...
Spiral::Spiral(const Film *film)
    : m_block_size(MTS_BLOCK_SIZE), m_size(film->size()) {

    m_n_blocks = Vector2i(
        (int) std::ceil(m_size.x() / static_cast<Float>(m_block_size)),
        (int) std::ceil(m_size.y() / static_cast<Float>(m_block_size)));
    m_n_total_blocks = m_n_blocks.x() * m_n_blocks.y();
    m_center = m_n_blocks / 2;
...
ref<ImageBlock> Spiral::next_block() {
    ...
    Vector2i size(std::min(static_cast<typename Vector2i::Value>(m_block_size),
                           m_size.x() - offset.x()),
                  std::min(static_cast<typename Vector2i::Value>(m_block_size),
                           m_size.y() - offset.y()));
