...
MTS_VARIANT Mesh<Float, Spectrum>::Mesh(
    ...
    for (size_t tri_loop_id = 0; tri_loop_id < loop_tri_count; tri_loop_id++) {
        ...
        if (!(blender::ME_SMOOTH & face.flag)) {
            ...
            const ScalarVector3f e1 = face_points[1] - face_points[0];
            const ScalarVector3f e2 = face_points[2] - face_points[0];
        ...
        for (u_int i = 0; i < 3; i++) {
            ...
            if (blender::ME_SMOOTH & face.flag) {
                ...
                normal = ScalarNormal3f(vert.no[0], vert.no[1], vert.no[2]);
