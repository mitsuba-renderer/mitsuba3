 ...
 * - thin
   - |bool|
   - Specifies whether :monosp:`regular` Principled BSDF or 2D :monosp:`thin`
     BSDF is going to be used. (Default:False)
 * - base_color
   - |spectrum| or |texture|
 ...
 * - metallic
   - |texture| or |float|
   - The "metallicness" of the model. (Default:0.0) Only used in :monosp:`regular`.
 * - spec_trans
   - |texture| or |float|
   - Has different roles in 2 models.

     - :monosp:`regular`: Blends BRDF and BSDF major lobe. (1.0: only BSDF
       response, 0.0 : only BRDF response.) (Default: 0.0)
     - :monosp:`thin`: Blends diffuse and specular responses. (1.0: only
       specular response, 0.0 : only diffuse response.)(Default: 0.0)
 * - eta
   - Different for

     - :monosp:`regular`: |float|
 ...
 * - specular
   ...
   - Controls the Fresnel reflection coefficient. This parameter has one to one
     ...
     (Default:0.5) Only used in :monosp:`regular`.
 * - spec_tint
   - |texture| or |float|
 ...
 * - clearcoat
   ...
   - The rate of the secondary isotropic specular lobe. (Default:0.0)
     Only used in :monosp:`regular`.
 * - clearcoat_gloss
   - |texture| or |float|
   - Controls the roughness of the secondary specular lobe. Clearcoat response
     gets glossier as the parameter increases. (Default:0.0) Only used
     in :monosp:`regular`.
 * - diff_trans
   - |texture| or |float|
   - The fraction that the energy of diffuse reflection is given to the
     ...
     transmission) (Default:0.0) Only used in :monosp:`thin`.
 ...
 * - main_specular_sampling_rate
   - |float|
   - The rate of the main specular lobe in sampling. (Default:1.0) Only used in
     :monosp:`regular`.
 ...
 * - specular_reflectance_sampling_rate
   - |float|
   - The rate of the main specular reflection in sampling. (Default:1.0) Only
     used in :monosp:`thin`.
 * - specular_transmittance_sampling_rate
   - |float|
   - The rate of the main specular transmission in sampling. (Default:1.0) Only
     used in :monosp:`thin`.
 * - diffuse_transmittance_sampling_rate
   - |float|
   - The rate of the cosine hemisphere transmission in sampling. (Default:1.0)
     Only used in :monosp:`thin`.
...
The following XML snippet describes a material definition for :monosp:`regular`
configuration:
...
public:
    ...
    Disney(const Properties &props) : Base(props) {
        ...
        // Parameters that are used in both models (thin and regular)
        ...
        // Thin and regular models use different parameters.
        if (!m_thin) {
            ...
            /* Thin material can also take texture parameters for eta while
               regular should not due to refraction! */
        ...
        } else { // regular lobe definitions
            // Diffuse reflection lobe in disney brdf lobe
            m_components.push_back(BSDFFlags::DiffuseReflection |
                                   BSDFFlags::FrontSide);
            // Clearcoat lobe (only exists in regular model)
        ...
        // Main specular reflection (same flags for both thin and regular)
        m_components.push_back(BSDFFlags::GlossyReflection |
                               BSDFFlags::FrontSide | BSDFFlags::BackSide |
                               BSDFFlags::Anisotropic);
    ...
    /**
     * \brief  Modified fresnel function for disney regular material. It blends
    ...
    thin_fresnel(const Float &F_dielectric, const Float &spec_tint,
        ...
        if (m_has_spec_tint) {
            ...
            F_schlick = calc_schlick<UnpolarizedSpectrum>(F0_spec_tint, cos_theta_i);
        }
    ...
    sample_regular(const BSDFContext &ctx, const SurfaceInteraction3f &si,
        ...
        bs.pdf = pdf_regular(ctx, si, bs.wo, active);
        active &= bs.pdf > 0.0f;
        Spectrum result = eval_regular(ctx, si, bs.wo, active);
        return { bs, result / bs.pdf & active };
    }

    std::pair<BSDFSample3f, Spectrum>
    sample_thin(const BSDFContext &ctx, const SurfaceInteraction3f &si,
                Float sample1, const Point2f &sample2, Mask active) const {
        Float cos_theta_i = Frame3f::cos_theta(si.wi);
        BSDFSample3f bs   = ek::zero<BSDFSample3f>();
        ...
        bs.pdf = pdf_thin(ctx, si, bs.wo, active);
        active &= bs.pdf > 0.0f;
        Spectrum result = eval_thin(ctx, si, bs.wo, active);
        return { bs, result / bs.pdf & active };
    }

    Spectrum eval_regular(const BSDFContext &ctx,
    ...
    Spectrum eval_thin(const BSDFContext &, const SurfaceInteraction3f &si,
    ...
    Float pdf_regular(const BSDFContext &, const SurfaceInteraction3f &si,
                      const Vector3f &wo, Mask active) const {
        Float cos_theta_i = Frame3f::cos_theta(si.wi);
        // Ignore perfectly grazing configurations.
        active &= ek::neq(cos_theta_i, 0.0f);
    ...
    Float pdf_thin(const BSDFContext &, const SurfaceInteraction3f &si,
                   const Vector3f &wo, Mask active) const {
        Float cos_theta_i = Frame3f::cos_theta(si.wi);
        // Ignore perfectly grazing configurations.
        active &= ek::neq(cos_theta_i, 0.0f);
    ...
    sample(const BSDFContext &ctx, const SurfaceInteraction3f &si,
        ...
        if (m_thin)
            return sample_thin(ctx, si, sample1, sample2, active);
        else
            return sample_regular(ctx, si, sample1, sample2, active);
    }
    ...
    Spectrum eval(const BSDFContext &ctx, const SurfaceInteraction3f &si,
        ...
        if (m_thin)
            return eval_thin(ctx, si, wo, active);
        else
            return eval_regular(ctx, si, wo, active);
    }
    ...
    Float pdf(const BSDFContext &ctx, const SurfaceInteraction3f &si,
        ...
        if (m_thin)
            return pdf_thin(ctx, si, wo, active);
        else
            return pdf_regular(ctx, si, wo, active);
    }

    void traverse(TraversalCallback *callback) override {
        // Only the corresponding model's parameters are traversed.
        if (!m_thin) { // regular traversed parameters
    ...
    std::string to_string() const override {
        ...
        if (m_thin) {
            ...
            oss << "Regular Principled BSDF :" << std::endl
                << "base_color: " << m_base_color << "," << std::endl
                << "spec_trans: " << m_spec_trans << "," << std::endl
                << "anisotropic: " << m_anisotropic << "," << std::endl
                << "roughness: " << m_roughness << "," << std::endl
    ...
    MTS_DECLARE_CLASS()
private:
    /// Whether the used model is 2D thin or regular principled-BSDF.
    bool m_thin;
    /// Parameters that are both used in regular and thin.
    ...
    /// Parameters peculiar to regular BSDF
    ...
    /// Sampling rates that are both used in regular and thin
    ScalarFloat m_diff_refl_srate;

    /// Sampling rates peculiar to the regular model
