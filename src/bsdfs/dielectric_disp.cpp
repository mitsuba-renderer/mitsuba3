 ...
 * - int_ior
   - |float| or |string| or |spectrum| or |texture|
...
class SmoothDielectricDisp final : public BSDF<Float, Spectrum> {
public:
    MI_IMPORT_BASE(BSDF, m_flags, m_components)
    MI_IMPORT_TYPES(Texture)

    SmoothDielectricDisp(const Properties &props) : Base(props) {
        m_dispersion = props.get<bool>("dispersion", false);

        auto parse_ior = [&](ref<Texture> &output,
                             const char *prop_name,
                             const char *default_ior_material) {
            if (!props.has_property(prop_name) ||
                props.type(prop_name) != Properties::Type::Object) {

                if (props.type(prop_name) == Properties::Type::Float &&
                    (props.get<float>(prop_name) < 0))
                    Throw("The interior and exterior indices of refraction must"
                          " be positive!");

                output = lookup_ior<Texture>(
                    props, prop_name, default_ior_material, m_dispersion);
            } else {
                output = props.texture<Texture>(prop_name);
            }
        };

        parse_ior(m_int_ior, "int_ior", "bk7");
        parse_ior(m_ext_ior, "ext_ior", "air");
    ...
    void traverse(TraversalCallback *callback) override {
        callback->put_parameter("int_ior", m_int_ior, +ParamFlags::NonDifferentiable);
        callback->put_parameter("ext_ior", m_ext_ior, +ParamFlags::NonDifferentiable);
    ...
    std::pair<BSDFSample3f, Spectrum> sample(const BSDFContext &ctx,
        ...
        UnpolarizedSpectrum eta = eval_eta(si, active);
        float eta_ = 0.f;
        ...
        if constexpr (is_polarized_v<Spectrum>) {
            ...
            float eta_ = 0.f;
            Spectrum R = mueller::specular_reflection(UnpolarizedSpectrum(cos_theta_o_hat), UnpolarizedSpectrum(eta_)),
                     T = mueller::specular_transmission(UnpolarizedSpectrum(cos_theta_o_hat), UnpolarizedSpectrum(eta_));
    ...
    std::string to_string() const override {
        ...
        oss << "SmoothDielectricDisp[" << std::endl;
        if (m_specular_reflectance)
        ...
        oss << "  int_ior = " << m_int_ior << "," << std::endl;
        oss << "  ext_ior = " << m_ext_ior << std::endl
            << "]";
        return oss.str();
    }
...
private:
    MI_INLINE UnpolarizedSpectrum eval_eta(const SurfaceInteraction3f &si,
                                           Mask active = true) const {
        return m_int_ior->eval(si, active) / m_int_ior->eval(si, active);
    }

private:
    bool m_dispersion;
    ref<Texture> m_int_ior;
    ref<Texture> m_ext_ior;
    ref<Texture> m_specular_reflectance;
    ref<Texture> m_specular_transmittance;
};

MI_IMPLEMENT_CLASS_VARIANT(SmoothDielectricDisp, BSDF)
MI_EXPORT_PLUGIN(SmoothDielectricDisp, "Smooth dielectric")
