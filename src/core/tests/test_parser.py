...
def test11_duplicate_id(variant_scalar_rgb):
    ...
    with pytest.raises(RuntimeError, match='duplicate ID: "my_id"'):
...
def test37_xml_include_error_chain(variant_scalar_rgb, tmp_path):
    ...
    error_msg = error_string(excinfo.value).replace(str(tmp_path) + "/", "")
...
def test53_instantiation_failure_location_xml(variant_scalar_rgb):
    ...
    expected = 'Error while loading string (line 2, col 10): failed to instantiate shape plugin of type "sphere": \nThe property "radius" has the wrong type (expected float, got string)'
    assert error_string(excinfo.value) == expected
...
def test54_instantiation_failure_location_dict(variant_scalar_rgb):
    ...
    expected = 'Error while loading dictionary node "myshape": failed to instantiate shape plugin of type "sphere": \nThe property "radius" has the wrong type (expected float, got string)'
    assert error_msg == expected
...
def test55_dict_auto_id_generation(variant_scalar_rgb):
    ...
    # Check that auto-generated IDs match the dictionary keys
    assert state.nodes[1].props.id() == "my_shape"
    assert state.nodes[2].props.id() == "my_shape.nested_emitter"  # New parser uses full path
...
def test60_default_parameter_values(variant_scalar_rgb):
    ...
    # Test 4: Unused default parameter should trigger error
    ...
    # With default config (unused_parameters = Error), this should fail
    with pytest.raises(RuntimeError) as excinfo:
        mi.parser.parse_string(config, xml_unused)
    error_msg = error_string(excinfo.value)
    assert "Found unused parameters" in error_msg
    assert "$unused_param=123" in error_msg
...
def test62_transform_relocate(variant_scalar_rgb, tmp_path):
    ...
    <scene version="2.1.0">
        <texture type="bitmap" name="wood_tex">
            <string name="filename" value="{texture1}"/>
        </texture>

        <texture type="bitmap" name="metal_tex">
            <string name="filename" value="{texture2}"/>
        </texture>

        <shape type="ply" name="bunny_shape">
            <string name="filename" value="{mesh1}"/>
        </shape>

        <shape type="obj" name="teapot_shape">
            <string name="filename" value="{mesh2}"/>
        </shape>
        ...
        <shape type="ply" name="bunny2_shape">
            <string name="filename" value="{duplicate_mesh2}"/>
        </shape>
    </scene>
    '''
    ...
    <scene version="2.1.0">
        ...
        <shape type="ply" name="existing_shape">
            <string name="filename" value="''' + str(mesh3) + '''"/>
        </shape>
    </scene>
    '''
    ...
    for node in state2.nodes:
        if "filename" in node.props:
            filename = node.props.get("filename")
            relocated_filenames.append(filename)
