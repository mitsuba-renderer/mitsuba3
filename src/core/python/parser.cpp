    ...
    if (type == "rgb") {
        ...
        if (value.is_valid()) {
            ...
            if (nb::try_cast<double>(value, value_d)) {
                ...
                spec.wavelengths.reserve(size);
                spec.values.reserve(size);
                ...
                for (nb::handle entry: value) {
                    ...
                    if (!nb::try_cast(entry[0], f0))
                        Throw("[%s] invalid wavelength '%s' in spectrum.",
                              path, nb::str(entry[0]).c_str());

                    if (!nb::try_cast(entry[1], f1))
                        Throw("[%s] invalid value '%s' in spectrum.",
                              path, nb::str(entry[1]).c_str());

                    spec.wavelengths.push_back(f0);
                    spec.values.push_back(f1);
                }
            } else {
...
static ParameterList convert_param_list(const nb::kwargs &kwargs) {
    ...
    for (auto [k, v] : kwargs)
        params.emplace_back(nb::cast<std::string>(k), nb::cast<std::string>(v));
    return params;
}
