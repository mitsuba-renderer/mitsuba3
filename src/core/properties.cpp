            ...
            if (spectrum.is_uniform()) {
                ...
                if (is_spectral) {
                    ...
                    Color3f color = spectrum_list_to_srgb(
                        ...
                        /* d65 */ !emitter || unbounded) * MI_CIE_Y_NORMALIZATION;

                    // Values should be scaled so that integrating the spectrum
                    // against the CIE curves and converting to sRGB yields
                    // (1, 1, 1) for D65 in non-spectral modes.
                    if (!is_spectral)
                        color *= MI_CIE_Y_NORMALIZATION;
...
Properties::Spectrum::Spectrum(const fs::path &file_path) {
    ...
    FILE *file = fopen(file_path.native().c_str(), "r");
    if (!file)
