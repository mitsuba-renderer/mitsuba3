...
MI_VARIANT
AnimatedTransform<Float, Spectrum>::AnimatedTransform(const ScalarTransform &trafo) {
...
MI_VARIANT
AnimatedTransform<Float, Spectrum>::AnimatedTransform(const std::map<ScalarFloat, ScalarTransform> &keyframes) {
    if (keyframes.size() == 1) {
...
MI_VARIANT auto AnimatedTransform<Float, Spectrum>::eval(Float time) const -> Transform {
...
MI_VARIANT auto AnimatedTransform<Float, Spectrum>::eval_scalar(ScalarFloat time) const -> ScalarTransform {
...
MI_VARIANT void AnimatedTransform<Float, Spectrum>::parameters_changed(const std::vector<std::string> &keys) {
...
MI_VARIANT void AnimatedTransform<Float, Spectrum>::add_keyframe(ScalarFloat time, const ScalarTransform &trafo) {
    auto [S, Q, T] = dr::transform_decompose(trafo.matrix);

    if (dr::abs(S[0][1]) > 1e-6 || dr::abs(S[0][2]) > 1e-6 || dr::abs(S[1][0]) > 1e-6 ||
        dr::abs(S[1][2]) > 1e-6 || dr::abs(S[2][0]) > 1e-6 || dr::abs(S[2][1]) > 1e-6)
