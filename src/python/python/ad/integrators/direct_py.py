        ...
        with dr.suspend_grad(when=not primal):
            ...
            with dr.resume_grad(when=not primal):
                ...
                if dr.hint(not primal, mode='scalar'):
                    ...
                    #active_diff_em = ( # FIXME
                    #    active_em &
                    #    mi.has_flag(ds_em.emitter.flags(), mi.EmitterFlags.SpatiallyVarying) &
                    #    mi.has_flag(ds_em.emitter.flags(), mi.EmitterFlags.Surface)
                    #)
                    active_diff_em = active_em

                    # Visibility is already accounted for, we can move the ray
                    # origin closer to the surface
                    # TODO: #o = dr.detach(si.p) #d = ds_em.p - o #dist = dr.norm(d) #d /= dist; ## todo: detach origin ? #ray_em = mi.Ray3f(o, d, dist * (1 - mi.math.ShadowEpsilon), si.time, si.wavelengths)

                    ray_em = si.spawn_ray_to(ds_em.p)
                    #ray_em.o = dr.fma(ray_em.d, ray_em.maxt, ray_em.o) # FIXME
                    ...
                    si_em = scene.ray_intersect(ray_em,
                                                ...
                                                coherent=False,
                                                reorder=False,
                                                active=active_diff_em)

                    ds_em_diff = mi.DirectionSample3f(scene, si_em, si)
                    #ds_em_diff = dr.select(active_diff_em, ds_em_diff, dr.zeros(mi.DirectionSample3f)) #FIXMe
                    #ds_em_diff.d = dr.select(~is_infinite, ds_em_diff.d, ds_em.d) #FIXME
