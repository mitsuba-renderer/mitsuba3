...
class ChiSquareTest:
    ...
    Args:
        domain: An implementation of the domain interface (`SphericalDomain`,
            etc.), which transforms between the parameter and target domain of
            the distribution.

        sample_func: An importance sampling function which maps an array of
            uniform variates of size ``[sample_dim, sample_count]`` to an
            array of ``sample_count`` samples on the target domain.

        pdf_func: Function that is expected to specify the probability
            density of the samples produced by ``sample_func``. The test will
            try to collect sufficient statistical evidence to reject this
            hypothesis.

        sample_dim: Number of random dimensions consumed by ``sample_func``
            per sample. The default value is ``2``.

        sample_count: Total number of samples to be generated. The test will
            have more evidence as this number tends to infinity. The default
            value is ``1000000``.

        res: Vertical resolution of the generated histograms. The horizontal
            resolution will be calculated as ``res * domain.aspect()``. The
            default value of ``101`` is intentionally an odd number to
            prevent issues with floating point precision at sharp boundaries
            that may separate the domain into two parts (e.g. top hemisphere
            of a sphere parameterization).

        ires: Number of horizontal/vertical subintervals used to numerically
            integrate the probability density over each histogram cell (using
            the trapezoid rule). The default value is ``4``.

        seed: Seed value for the PCG32 random number generator used in the
            histogram computation. The default value is ``0``.

    Attributes:
        messages (str): The implementation may generate a number of messages
            while running the test, which can be retrieved via this
            attribute.

        histogram: The histogram array is populated by the
            `tabulate_histogram` method and stored in this attribute.

        pdf: The probability density function array is populated by
            the `tabulate_pdf` method and stored in this attribute.

        p_value (float): The p-value of the test is computed in the `run`
            method and stored in this attribute.
    """
    ...
    def run(self, significance_level=0.01, test_count=1, quiet=False):
        """
        Run the :math:`\\chi^2` test

        Args:
            significance_level: Denotes the desired significance level (e.g.
                0.01 for a test at the 1% significance level).

            test_count: Specifies the total number of statistical tests run
                by the user. This value will be used to adjust the provided
                significance level so that the combination of the entire set
                of tests has the provided significance level.
...
def BSDFAdapter(bsdf_type, extra, wi=[0, 0, 1], uv=[0.5, 0.5], ctx=None):
    """
    Adapter to test BSDF sampling using the :math:`\\chi^2` test.

    Args:
        bsdf_type: Name of the BSDF plugin to instantiate.

        extra: Additional XML used to specify the BSDF's parameters, or a
            Python dictionary as used by the `mitsuba.load_dict` routine.

        wi: Incoming direction, in local coordinates.
    """
...
def EmitterAdapter(emitter_type, extra):
    """
    Adapter to test Emitter sampling using the :math:`\\chi^2` test.

    Args:
        emitter_type: Name of the emitter plugin to instantiate.

        extra: Additional XML used to specify the emitter's parameters, or a
            Python dictionary as used by the `mitsuba.load_dict` routine.
    """
...
def PhaseFunctionAdapter(phase_type, extra, wi=[0, 0, 1], ctx=None):
    """
    Adapter to test phase function sampling using the :math:`\\chi^2` test.

    Args:
        phase_type: Name of the phase function plugin to instantiate.

        extra: Additional XML used to specify the phase function's
            parameters, or a Python dictionary as used by the
            `mitsuba.load_dict` routine.
