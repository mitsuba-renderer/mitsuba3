...
def write_tensor(filename, align=8, **kwargs):
    ...
    with open(filename, 'wb') as f:
        ...
        # Maps to Struct.Type field in Mitsuba
        dtype_map = {
            np.uint8 : 1,
            np.int8 : 2,
            np.uint16 : 3,
        ...
        for k, v in fields.items():
            ...
            # Field identifier
            label = k.encode('utf8')
            f.write(pack('<H', len(label)))
            f.write(label)

            # Field dimension
            f.write(pack('<H', v.ndim))

            found = False
            for dt in dtype_map.keys():
                if dt == v.dtype:
                    found = True
                    f.write(pack('B', dtype_map[dt]))
                    break
            if not found:
...
def load_tensor(filename):
    ...
    with open(filename, 'rb') as f:
        ...
        # Maps from Struct.Type field in Mitsuba
        dtype_map = {
            1: np.uint8,
            2: np.int8,
            3: np.uint16,
