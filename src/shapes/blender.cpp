    ...
    BlenderMesh(const Properties &props) : Base(props) {
        ...
        for (size_t tri_loop_id = 0; tri_loop_id < loop_tri_count; tri_loop_id++) {
            ...
            if (!smooth_face && !m_face_normals) {
                ...
                normal = m_to_world->eval_scalar(0.f) * dr::cross(e1, e2);
            ...
            for (int i = 0; i < 3; i++) {
                ...
                if (smooth_face && !m_face_normals) {
                    if (version <= Version(3, 0, 0)) {
                        ...
                        normal = m_to_world->eval_scalar(0.f) * InputNormal3f(no[0], no[1], no[2]);
                    } else {
                ...
                if (map_entry->is_init && map_entry->key == vert_key) {
                    ...
                    InputPoint3f pt = m_to_world->eval_scalar(0.f) * face_points[i];
