...
public:
    ...
    EllipsoidsData(const Properties &props) {
        if (props.has_property("filename")) {
            ...
            TensorXf &data = props.get_any<TensorXf>("data");
            ...
            TensorXf32 &centers = props.get_any<TensorXf32>("centers");
            TensorXf32 &scales  = props.get_any<TensorXf32>("scales");
            TensorXf32 &quats   = props.get_any<TensorXf32>("quaternions");
        ...
        if (!unqueried.empty()) {
            for (auto &key : unqueried) {
                if (key == "shell")
                    continue;
                TensorXf &tensor = props.get_any<TensorXf>(key);
                if (tensor.ndim() != 2)
