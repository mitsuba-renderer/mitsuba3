...
MTS_PY_EXPORT(math) {
    ...
    m.attr("OneMinusEpsilon") = py::cast(ek::OneMinusEpsilon<Float>);
    m.attr("RecipOverflow")   = py::cast(math::RecipOverflow<Float>);
    m.attr("Epsilon")         = py::cast(ek::Epsilon<Float>);
    m.attr("Infinity")        = py::cast(ek::Infinity<Float>);
    ...
    m.def("morton_decode2", vectorize(&ek::morton_decode<Array<UInt32, 2>>), "m"_a);
    m.def("morton_decode3", vectorize(&ek::morton_decode<Array<UInt32, 3>>), "m"_a);
    m.def("morton_encode2", vectorize(&ek::morton_encode<Array<UInt32, 2>>), "v"_a);
    m.def("morton_encode3", vectorize(&ek::morton_encode<Array<UInt32, 3>>), "v"_a);
