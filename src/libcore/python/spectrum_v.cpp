...
MTS_PY_EXPORT(Spectrum) {
    ...
    if constexpr (is_spectral_v<Spectrum>) {
        m.def("spectrum_to_xyz", vectorize(&spectrum_to_xyz<Float, ek::array_size_v<Spectrum>>),
              "value"_a, "wavelengths"_a, "active"_a = true, D(spectrum_to_xyz));

        m.def("sample_shifted",
            vectorize(
                py::overload_cast<const ek::value_t<Array<Float, ek::array_size_v<Spectrum>>> &>(
                    &math::sample_shifted<Array<Float, ek::array_size_v<Spectrum>>>)),
            "sample"_a);
    ...
    if constexpr (ek::is_cuda_array_v<Float>) {
        ...
        pybind11_type_alias<Array<Float, ek::array_size_v<Spectrum>>, Spectrum>();
    }
}
