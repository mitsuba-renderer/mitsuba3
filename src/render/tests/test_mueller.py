...
def test01_depolarizer(variant_scalar_rgb):
    ...
    assert dr.allclose(depolarizer(.8) @ Array4f([1, .5, .5, .5]), [.8, 0, 0, 0])
...
def test02_rotator(variant_scalar_rgb):
    ...
    assert dr.allclose(rotator( 45 * dr.Pi/180) @ Array4f([1, 1, 0, 0]), [1, 0, -1, 0], atol=1e-5)
    # .. rotate the Stokes frame by -45˚
    assert dr.allclose(rotator(-45 * dr.Pi/180) @ Array4f([1, 1, 0, 0]), [1, 0, +1, 0], atol=1e-5)
...
def test05_specular_reflection(variant_scalar_rgb):
    ...
    assert dr.allclose(specular_reflection(1, 1.5),   linear_retarder(dr.Pi)*0.04)
    assert dr.allclose(specular_reflection(1, 1/1.5), linear_retarder(dr.Pi)*0.04)

    # Just before Brewster's angle: 180 deg phase shift
    M = specular_reflection(dr.cos(dr.atan(1.5) - 0.01), 1.5)
    assert M[0, 0] > 0 and M[1, 1] > 0 and M[2, 2] < 0 and M[3, 3] < 0
    M = specular_reflection(dr.cos(dr.atan(1/1.5) - 0.01), 1/1.5)
    assert M[0, 0] > 0 and M[1, 1] > 0 and M[2, 2] < 0 and M[3, 3] < 0

    # Brewster's angle: perfect linear polarization
    M = np.zeros((4, 4))
    M[0:2, 0:2] = 0.0739645
    assert dr.allclose(specular_reflection(dr.cos(dr.atan(1/1.5)), 1/1.5), M, atol=1e-6)
    assert dr.allclose(specular_reflection(dr.cos(dr.atan(1.5)), 1.5), M, atol=1e-6)
    ...
    assert dr.allclose(specular_reflection(0, 1.5),   dr.identity(Matrix4f), atol=1e-6)
    assert dr.allclose(specular_reflection(0, 1/1.5), dr.identity(Matrix4f), atol=1e-6)
...
def test06_specular_transmission(variant_scalar_rgb):
    ...
    assert dr.allclose(specular_transmission(1, 1.5), dr.identity(Matrix4f) * 0.96, atol=1e-5)
    assert dr.allclose(specular_transmission(1, 1/1.5), dr.identity(Matrix4f) * 0.96, atol=1e-5)
    assert dr.allclose(specular_transmission(1e-7, 1.5), dr.zero(Matrix4f), atol=1e-5)
    assert dr.allclose(specular_transmission(1e-7, 1/1.5), dr.zero(Matrix4f), atol=1e-5)
    assert dr.allclose(specular_transmission(0, 1.5), dr.zero(Matrix4f), atol=1e-5)
    assert dr.allclose(specular_transmission(0, 1/1.5), dr.zero(Matrix4f), atol=1e-5)

    ref = dr.scalar.Matrix4f([[ 0.9260355 , -0.07396451,  0.        ,  0.        ],
                              [-0.07396451,  0.9260355 ,  0.        ,  0.        ],
                              [ 0.        ,  0.        ,  0.92307705,  0.        ],
                              [ 0.        ,  0.        ,  0.        ,  0.92307705]])
    assert dr.allclose(specular_transmission(dr.cos(dr.atan(1/1.5)), 1/1.5), ref)
...
def test08_rotate_mueller_basis(variant_scalar_rgb):
    ...
    M_rotated_element = rotated_element(-45 * dr.Pi/180, M)
