...
def write_kernels(*args, output_dir='kernels', scene_fname=None):
    ...
    fields = ('type', 'size', 'input_count', 'output_count', 'operation_count',
              'hash', 'codegen_time', 'backend_time', 'execution_time',
              'uses_optix', 'cache_hit', 'cache_disk')

    os.makedirs(output_dir, exist_ok=True)
    for i, h in enumerate(args):
        for j, entry in enumerate(h):
            fname = os.path.join(output_dir, f'run_{i}_kernel_{j}.ptx')
            info = {k: getattr(entry, k) for k in fields}

            with open(fname, 'w') as f:
                meta = '\n// '.join(json.dumps(info, indent=2, default=str).split('\n'))
...
def test03_kernel_launches_optimization(variants_all_ad_rgb):
    ...
    for it in range(3):
        ...
        with dr.kernel_history([dr.KernelType.JIT]) as history_primal:
        ...
        with dr.kernel_history([dr.KernelType.JIT]) as history_loss:
            dr.enable_grad(image)
            ob_val = dr.sum(dr.square(image - image_ref)) / len(image)
            dr.backward(ob_val)
        ...
        with dr.kernel_history([dr.KernelType.JIT]) as history_adjoint:
        ...
        with dr.kernel_history([dr.KernelType.JIT]) as history_step:
            opt.step()
        ...
        with dr.kernel_history([dr.KernelType.JIT]) as history_update:
            opt.update()

        assert len(history_update) == 0

        histories.append([*history_primal, *history_loss, *history_adjoint,
                          *history_step])
