...
def test04_srgb_d65(variant_scalar_spectral, np_rng):
    ...
    from mitsuba.core import load_string, MI_WAVELENGTH_SAMPLES
    from mitsuba.render import PositionSample3f, SurfaceInteraction3f
    import numpy as np


    wavelengths = np.linspace(300, 800, MI_WAVELENGTH_SAMPLES)
...
def test05_sample_rgb_spectrum(variant_scalar_spectral):
    ...
    from mitsuba.core import MI_CIE_MIN, MI_CIE_MAX
    ...
    if (MI_CIE_MIN == 360 and MI_CIE_MAX == 830):
        spot_check(0.1, 424.343, 465.291)
        spot_check(0.5, 545.903, 254.643)
        spot_check(0.8, 635.381, 400.432)
        # PDF is zero outside the range
        assert pdf_rgb_spectrum(MI_CIE_MIN - 10.0) == 0.0
        assert pdf_rgb_spectrum(MI_CIE_MAX + 0.5)  == 0.0
...
def test06_rgb2spec_fetch_eval_mean(variant_scalar_spectral):
    ...
    from mitsuba.core import MI_CIE_MIN, MI_CIE_MAX, MI_WAVELENGTH_SAMPLES
    import numpy as np
