...
void OptixAccel<Float, Spectrum>::init(Scene<Float, Spectrum> *scene,
    ...
    if (other_scene) {
        ...
        const MiOptixConfig &config =
            init_optix_config(optix_prim_flags(sd), optix_uses_motion_blur(sd));
...
static void optix_switch_config(Scene<Float, Spectrum> *scene,
    ...
    const MiOptixConfig &config =
        init_optix_config(optix_prim_flags(sd), uses_motion_blur);
    ...
    // than updated in place. Drop the old registration and its handle. The
    // intersection bindings of the old registration keep it alive until
    // the record update below replaces them.
    sbt_handle = 0;
    (void) UInt32::steal(s.sbt_jit_index);
    s.sbt_jit_index = jit_optix_configure_sbt(&s.sbt, config.pipeline_jit_index);

    // The hit records need the same treatment. The record count is unchanged,
    // so they are rewritten into the existing table. Their headers are
    // packed from the program groups of the new pipeline, and the bindings
    // of custom shapes reference the new registration.
    optix_write_sbt_records(s, sd, config);
    s.config_key = config.key;
    s.context = config.context;
    s.pipeline_jit_index = config.pipeline_jit_index;
