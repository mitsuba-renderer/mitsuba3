...
struct MiOptixConfig {
    ...
    char *custom_shapes_is_pg_name[MI_OPTIX_SHAPE_TYPE_COUNT]; /// Intersection program names
...
size_t init_optix_config(uint32_t optix_config_shapes) {
    ...
    for (size_t i = 0; i < MI_OPTIX_SHAPE_TYPE_COUNT; i++) {
        ...
        config.custom_shapes_is_pg_name[i] = is_name;
        ...
        pgd[pg_count - 1].hitgroup.entryFunctionNameIS = config.custom_shapes_is_pg_name[i];
    ...
    if constexpr (dr::is_cuda_v<Float>) {
        ...
        for (size_t j = 0; j < MI_OPTIX_CONFIG_COUNT; j++) {
            ...
            if (config.pipeline_jit_index) {
                ...
                for (size_t i = 0; i < MI_OPTIX_SHAPE_TYPE_COUNT; i++)
                    free(config.custom_shapes_is_pg_name[i]);
