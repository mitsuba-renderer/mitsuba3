...
MI_VARIANT void Mesh<Float, Spectrum>::copy_attributes(const Mesh &other) {
    if (other.m_vertex_count != m_vertex_count ||
        other.m_face_count != m_face_count)
        Throw("copy_attributes(): mesh \"%s\" and \"%s\" differ in their "
              "vertex or face count.", m_filename, other.m_filename);

    for (const auto &[name, attr] : other.m_mesh_attributes)
        if (!m_mesh_attributes.insert({ name, attr }).second)
            Throw("copy_attributes(): attribute \"%s\" already exists.", name);
}
