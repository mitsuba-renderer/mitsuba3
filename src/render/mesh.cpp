...
MI_VARIANT void Mesh<Float, Spectrum>::build_directed_edges() {
    ...
    if constexpr (dr::is_jit_v<Float>) {
        ...
        */
        ...
        dr::scoped_set_flag scope_debug(JitFlag::Debug, true);

        auto [_, swapped] =
            dr::scatter_cas(V2E, UInt32(INVALID_DEDGE), edge_id, v1, !invalid_edges);

        dr::eval(V2E, swapped); //FIXME improve ?

        Mask has_multiple_edges_at_v1 = !swapped & !invalid_edges;
        ...
        LoopState ls {
            dr::gather<UInt32>(V2E, v1, has_multiple_edges_at_v1), // Old is valid too?
            has_multiple_edges_at_v1
        };
        dr::tie(ls) = dr::while_loop(
            ...
            [&tmp, &edge_id](LoopState &ls) {
                ...
                dr::eval(swapped, tmp[1]);
                //ls.next_edge_id = old;
                ls.next_edge_id = dr::gather<UInt32>(tmp[1], ls.next_edge_id);
                ls.active &= !swapped;
            }
        );
        ...
        UInt32 edge_id_opp = dr::full<UInt32>(INVALID_DEDGE, m_face_count * 3);
        ...
        dr::tie(ls2) = dr::while_loop(
            ...
            [&tmp, &v1, &v2, &non_manifold](LoopState2 &ls) {
                ...
                // Set opposite edge
                ls.edge_id_opp[found_edge & invalid_opp & ls.active] = ls.it;
                ...
                ls.edge_id_opp[found_edge & !invalid_opp & ls.active] = INVALID_DEDGE;
                ls.active[found_edge & !invalid_opp & ls.active] &= false;

                // Move to next edge of vertex
                ls.it = dr::gather<UInt32>(tmp[1], ls.it, ls.active);
