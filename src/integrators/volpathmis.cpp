...
public:
    ...
    std::pair<Spectrum, Mask> sample(const Scene *scene,
        ...
        while (loop(ek::detach(active))) {
            ...
            if (ek::any_or<true>(active_medium)) {
                ...
                UnpolarizedSpectrum prob_emission = ek::select(ek::hmean(mi.radiance) > 0, mi.sigma_t, 0.f);
    ...
    sample_emitter(const Interaction3f &ref_interaction, const Scene *scene,
        ...
        auto [ds, emitter_sample_weight] = scene->sample_emitter_direction(ref_interaction, sampler->next_2d(active), sampler->next_1d(active), false, active);
