 ...
 * - strict_normals
   - |bool|
   - Be strict about potential inconsistencies involving shading normals? See the description below
     for details. (Default: no, i.e. |false|)
 * - hide_emitters
   - |bool|
...
Like the :ref:`integrator-direct` plugin, the path tracer internally relies on multiple importance
...
The :code:`strict_normals` parameter specifies the intended behavior when such cases arise. The
