...
public:
    ...
    void set_sample_count(uint32_t spp) override {
        ...
        m_resolution = 1;
        while ((dr::square(m_resolution)*m_resolution) < spp)
            m_resolution++;

        if (spp != (dr::square(m_resolution)*m_resolution))
            Log(Warn, "Sample count should be cube and power of two, rounding to %i", dr::square(m_resolution)*m_resolution);

        m_sample_count = dr::square(m_resolution)*m_resolution;
