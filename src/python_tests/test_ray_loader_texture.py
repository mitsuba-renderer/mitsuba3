...
scene_dict = {
    ...
    'shape': {
        ...
        'bsdf': {
            ...
            'reflectance': {
                'type': 'bitmap',
                'filename': '../../tutorials/scenes/textures/flower_photo_downscale.jpeg',
                'format': 'variant',
                # 'raw': True,
                'to_uv': T().scale([1, -1, 1]),
            }
    ...
    'sensor1': {
        'type': 'perspective',
        'fov': 90,
        'to_world': T().look_at(target=[0.5, 0.5, 0], origin=[0.5, 0.5, 0.5], up=[0, 1, 0]),
        'film': {
            'type': 'ref',
            'id': 'film_box',
        },
...
for iteration in range(50):
    ...
    batch_image = mi.render(scene, params, sensor=flat_sensor, spp=4, spp_grad=1, seed=iteration)

    # Ray loader is a kind of sampling, so we need to multiply the loss by the
    # pixel batch multiplier
    batch_loss = (
    ...
    if (iteration + 1) % 5 == 0:
        mi.Bitmap(params[key]).write(f'test_texture_rayloader_iter_{iteration+1:03d}.exr')
