...
def test09_resampler_filter_only(variant_scalar_rgb):
    ...
    b = np.zeros(3, dtype=float_dtype)
    resampler.resample(a, 1, b, 1, 1)
    assert ek.allclose(b[0], (G(0) * a[0] + G(1) * (a[1] + a[2])) / (G(0) + 2*G(1)), atol=1e-4)
    assert ek.allclose(b[1], (G(0) * a[1] + G(1) * (a[0] + a[2])) / (G(0) + 2*G(1)), atol=1e-4)
    assert ek.allclose(b[2], (G(0) * a[2] + G(1) * (a[0] + a[1])) / (G(0) + 2*G(1)), atol=1e-4)
